...
/**
 * Class MensajeriaCargamos
 * @package App\ClaroEnvios\Mensajerias
 * @version 2.0
 * @author Roberto Martinez
 */
class MensajeriaCargamos extends MensajeriaMaster implements MensajeriaCotizable
{
    ...
    private $arrayLabelUrl = [
        'PRODUCCION'=>"https://api.platform.cargamos.com/v1/",
        'TEST'=>"https://api.unstable.cargamos.com/v1/"
    ...
    /*public function generarGuia(GuiaMensajeriaTO $guiaMensajeriaTO){
        try {
            ...
            $file = $this->generateFile($delivery->result->deliveries['0']->tracking_label_pdf);

            $tracking_id = $delivery->result->deliveries['0']->tracking_id;
            ...
            $array['infoExtra']=[
                ...
                // 'tracking_link' =>$this->url_tracking.$tracking_id
            ];
    ...
    private function makeRequest($data,$type, $method = 'POST',$bitacoraCotizacionId = null){
        ...
        try{
            ...
            if ($token) {
                ...
                $response = $this->requestCargamos($data, $type, $method,$bitacoraCotizacionId);
...
//            die(print_r($exception->getMessage().': '.$exception->getFile().' '.$exception->getLine()));
            if($exception->getCode() == 401){
                ...
                if($accesoComercio){
                    ...
                    $tokenCargamos = $this->getToken('client_credentials');
     ...
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    private function requestCargamos($data,$type,$method, $bitacoraCotizacionId = null){
        $token = $this->configuracion->get('token');
...
//            die(print_r($response));
    ...
    public function createDelivery(BitacoraMensajeriaDestinoTO $destino, GuiaMensajeriaTO $guiaMensajeriaTO, $origen)
    {
        try{
            ...
            $address = $this->limitar_cadena($destino->getCalle(), 180, 1);
            $address_1="";
            $address_2="";
            if (is_array($address)) {
                $address_1=$address["0"];
            ...
            $data = [
                "deliveries"=> [
                    [
                        "cargo" => [[
                            "cargo_id"=>  $this->pedido,
                            "cargo_type"=> ($this->tipo_paquete == "1") ? "BAG" : "BOX",
                        ...
                        "delivery_scheduling_parameters"=> [
                            "dropoff_by_date" => $timestamp,
                            "pickup_by_date" =>  $timestamp
                        ],
                        "destination_location"=> [
                            "address_line_1"=> $address_1 .' '.$destino->getNumero(),
                            "address_line_2"=> $address_2,
                            "administrative_area"=> $destino->getEstado(),
                            "contact_information"=> [
                              "customer_id"=> "",
                              "email"=> $destino->getEmail(),
                              "first_name"=> $this->clean($destino->getNombre()),
                              "last_name"=> $this->clean($destino->getApellidos()),
                              "phone"=> "+52".$destino->getTelefono()
                            ],
                            "country"=> "MX",
                            "locality"=> $this->clean($destino->getColonia()),
                            "notes"=> $this->limitar_cadena($destino->getReferencias() , 200, 2) ,
                            "postal_code"=> $this->codigo_postal_destino
                          ],
            ...
            $response =  $this->makeRequest($data,"deliveries");
